...
// // קבלת כל החדרים (Read - All)
// export const getAllRooms = async (req, res) => {
//   try {
//     // כאן אנחנו לא עושים populate כי זה יכביד על הרשימה הכללית
//     const rooms = await Room.find();
//     res.status(200).json(rooms);
//   } catch (err) {
//     res.status(500).json({ message: "שגיאה בשליפת חדרים", error: err.message });
...
// // קבלת חדר ספציפי עם המערכת שלו (Read - One + Populate)
// export const getRoomById = async (req, res) => {
//   try {
//     // הקסם של ה-Populate: כאן אנחנו "מדביקים" את השיבוצים בזיכרון
//     const room = await Room.findById(req.params.id).populate("allocations");
    
//     if (!room) return res.status(404).json({ message: "החדר לא נמצא" });
//     res.status(200).json(room);
//   } catch (err) {
//     res.status(500).json({ message: "שגיאה בשליפת נתוני חדר", error: err.message });
//   }
// };

// // עדכון פרטי חדר (Update)
// export const updateRoom = async (req, res) => {
//   try {
//     const updatedRoom = await Room.findByIdAndUpdate(
//       req.params.id,
//       { $set: req.body },
//       { new: true, runValidators: true } // מחזיר את האובייקט המעודכן ומריץ ולידציה
//     );
//     res.status(200).json(updatedRoom);
//   } catch (err) {
...
//     res.status(200).json({ message: "החדר נמחק בהצלחה" });
//   } catch (err) {
//     res.status(500).json({ message: "שגיאה במחיקת חדר", error: err.message });
//   }
// };